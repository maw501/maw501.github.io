...
\log P(X) &= \mathbb{E}_{z \sim Q} \left[\log P(X) \right] &\text{($P(X)$ not dep. on $z$)} \\[5pt]

&=\mathbb{E}_{z \sim Q}\left[\log \frac{P(X | z) P(z)}{P(z | X)}\right]  &\text{(Baye's rule)} \\[5pt]

&=\mathbb{E}_{z \sim Q}\left[\log \frac{P(X | z) P(z)}{P(z | X)} \frac{Q(z | X)}{Q(z| X)}\right]  &\text{(mul. by one)} \\[5pt]

&=\mathbb{E}_{z \sim Q}\left[\log P(X | z) \right]-\mathbb{E}_{z\sim Q}\left[\log \frac{Q(z |X)}{P(z)}\right] + \mathbb{E}_{z\sim Q}\left[\log \frac{Q(z|X)}{P(z|X)}\right]  \hspace{0.25cm} &\text{(by logs)} \\[5pt]

&= \mathbb{E}_{z \sim Q}\left[\log P(X|z)\right] - \mathcal{D}\left[Q(z|X) \| P(z)\right]+\mathcal{D}\left[Q(z|X) \| P(z|X)\right] &\text{(by defn. of KL)}
...
& \geq \mathbb{E}_{z \sim Q}\left[\log \left( \frac{P(X, z)}{Q(z | X)} \right) \right] \hspace{3cm} \text{(by Jensen's inequality)} \\[5pt]
