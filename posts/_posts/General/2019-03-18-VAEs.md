...
comments: true
---
This article will introduce VAEs, providing a synthesis of many of the excellent explanations that already exist. In particular we will derive the ELBO in three different ways as well as list some common questions that arose for me whilst understanding VAEs.
...
<!---
<hr class="with-margin">
<h4 class="header" id="overview">Post roadmap</h4>
...
For a vanilla VAE we are not thinking about the class labels $y$ but rather how to learn the data distribution $P(X)$ from which we will then be able to create new samples from. In particular we would like to be able to maximize the probability of $X$ under a generative process that assumes the latent variables, $z$. We do this be integrating/marginalising out the hidden variables $z$:
...
<blockquote class="tip">
<strong>Goal:</strong> Formally we will show that $\log P(X) \geq$ ELBO and so maximizing the ELBO (which is tractable) will also be maximizing $\log P(X)$ - our term of interest.
</blockquote>
...
There are several ways to derive the ELBO, we will show 3 variants, starting with the one shown in [Doersch](https://arxiv.org/abs/1606.05908).

<a name="elbo1"></a>
##### ELBO derivation 1: start with KL divergence

The start of the derivation from this angle can seem like a little bit like a leap of faith without the appropriate context. It's worth recalling that calculating the posterior of the latent variables $P(z \| X)$ is intractable (as it involves $P(X)$ which is an integral over $z$) and so variational inference attempts to find some approximate distribution that is as close to $P(z \| X)$ as possible.
...
The next thing to note is that given we are using logs (maximizing the log of something is the same as maximizing the thing itself) we can use Baye's rule to write:
...
$$\mathcal{D}[Q(z| X) \| P(z | X)]=\mathbb{E}_{z \sim Q}[\log Q(z| X)- \log P(X | z) - \log P(z) ] + \log P(X)$$

where we have moved $\log P(X)$ outside the expectation as it doesn't depend on $z$. Using the linearity of expectations

$$\mathbb{E}[A + B] = \mathbb{E}[A] + \mathbb{E}[B]$$

we have (after swapping sides for a few terms)

$$\log P(X) - \mathcal{D}[Q(z| X) \| P(z | X)] = \mathbb{E}_{z \sim Q}[ \log P(X | z)] - \underbrace{\mathbb{E}_{z \sim Q}[ \log Q(z| X)- \log P(z)]}_{\mathcal{D}[Q(z| X) \| P(z)]} $$
...
$$\underbrace{\log P(X)}_\text{want to max}
 ...
 \overbrace{\underbrace{\mathbb{E}_{z \sim Q}[ \log P(X | z)]}_\text{reconstruction loss}
...
<a name="elbo2"></a>
##### ELBO derivation 2: start with $\log P(X)$

An alternative way to derive the result we desire is to start directly from the term we wish to maximize, $P(X)$ and show this is greater than the ELBO. Recall that maximizing $\log P(X)$ is the same as maximizing $P(X)$ and also bear in mind the definition of the KL divergence in terms of expectations:

<div class="math">
\begin{align*}
\mathcal{D}(P \| Q) &= \int_{-\infty}^{\infty} P(x) \log \left(\frac{P(x)}{Q(x)}\right) dx \\[5pt]
...
We will detail the full derivation and then provide some explanation afterwards, the main body follows that shown in the [CS231n lecture](https://www.youtube.com/watch?v=5WoItGTWV54).

<div class="math">
\begin{align*}

\log P(X) &= \mathbb{E}_{z \sim Q} \left[\log P(X) \right] \hspace{1cm} \text{(as $P(X)$ does not depend on $z$)} \\[5pt]

&=\mathbb{E}_{z \sim Q}\left[\log \frac{P(X | z) P(z)}{P(z | X)}\right] \hspace{1cm} \text{(by Baye's rule)} \\[5pt]

&=\mathbb{E}_{z \sim Q}\left[\log \frac{P(X | z) P(z)}{P(z | X)} \frac{Q(z | X)}{Q(z| X)}\right] \hspace{1cm} \text{(multiply by one)} \\[5pt]

&=\mathbb{E}_{z \sim Q}\left[\log P(X | z) \right]-\mathbb{E}_{z\sim Q}\left[\log \frac{Q(z |X)}{P(z)}\right] + \mathbb{E}_{z\sim Q}\left[\log \frac{Q(z|X)}{P(z|X)}\right] \hspace{1cm} \text{(by logarithms)} \\[5pt]

&= \mathbb{E}_{z \sim Q}\left[\log P(X|z)\right] - \mathcal{D}\left[Q(z|X) \| P(z)\right]+\mathcal{D}\left[Q(z|X) \| P(z|X)\right] \hspace{1cm} \text{(by definition of KL)}

\end{align*}
</div>

this is exactly the same expression as we showed above in [ELBO derivation 1](#elbo1).

The above derivation uses very similar techniques as the first derivation we saw except this time the KL terms are introduced at the end as a way to rewrite the expectations. The derivation started by rewriting $\log P(X)$ as an expectation over $z$:

$$ \log P(X) = \mathbb{E}_{z \sim Q} \left[\log P(X) \right] $$

where the justification for doing this is that $\log P(X)$ has no dependency on $z$ and so, by the linearity of expectations, $\mathbb{E}[aX] = a \mathbb{E}[X]$ we have:

<div class="math">
\begin{align*}
\mathbb{E}_{z \sim Q} \left[\log P(X) \right] &= \log P(X) \, \mathbb{E}_{z \sim Q} \left[ 1 \right] \\[5pt]
&= \log P(X)
\end{align*}
</div>
...
This derivation is perhaps the most often cited and relies on [Jensen's inequality](https://en.wikipedia.org/wiki/Jensen%27s_inequality). Again we start from $\log P(X)$:

<div class="math">
\begin{align*}
\log P(X) &= \log \int P(X, z) \, dz \\[5pt]
&=\log \int P(X, z) \frac{Q(z | X)}{Q(z | X)} \, dz\\[5pt]

&=\log \left(\mathbb{E}_{z \sim Q}\left[\frac{P(X, z)}{Q(z | X)}\right]\right) \\[5pt]

& \geq \mathbb{E}_{z \sim Q}\left[\log \left( \frac{P(X, z)}{Q(z | X)} \right) \right] \hspace{1cm} \text{(by Jensen's inequality)} \\[5pt]

&= \mathbb{E}_{z \sim Q}[\log P(X, z)]-\mathbb{E}_{z \sim Q}[\log Q(z | X)] \\[7.5pt]

&= \mathbb{E}_{z \sim Q}[\log P(X | z)] + \mathbb{E}_{z \sim Q}[\log P(z)] -\mathbb{E}_{z \sim Q}[\log Q(z | X)] \\[7.5pt]

&= \mathbb{E}_{z \sim Q}[ \log P(X | z)] - \underbrace{\mathbb{E}_{z \sim Q}[ \log Q(z| X)- \log P(z)]}_{\mathcal{D}[Q(z| X) \| P(z)]} \\[5pt]

&=  \underbrace{\mathbb{E}_{z \sim Q}[ \log P(X | z)]
 - \mathcal{D}[Q(z|X) \| P(z)]}_\text{ELBO}

\end{align*}
</div>

and so again we have shown that $\log P(X) \geq$ ELBO. The above derivation as [usually presented](https://www.cs.princeton.edu/courses/archive/fall11/cos597C/lectures/variational-inference-i.pdf) is a bit shorter but we have continued on a few more lines to make the link to the ELBO and our previous derivations explicit.
...
<blockquote class="tip">
<strong>Note:</strong> ELBO stands for Evidence Lower BOund and refers to the fact the ELBO provides a lower bound for $ \log P(X)$, the log probability of the observed data.
</blockquote>
...
$$\mathcal{L} =
 \overbrace{\underbrace{ - \mathbb{E}_{z \sim Q}[ \log P(X | z)]}_\text{reconstruction loss}
 + \underbrace{\mathcal{D}[Q(z|X) \| P(z)]}_\text{KL loss}}^\text{-ve ELBO}
 $$
...
<hr class="with-margin">
<h4 class="header" id="QandA">Q and A section</h4>
